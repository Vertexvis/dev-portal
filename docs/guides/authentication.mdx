...
:::note
If your application already authenticates users via an OIDC-compliant identity
provider (such as Okta, Azure AD, or Auth0), consider using the [Token
Exchange](#token-exchange-rfc8693) flow instead. This allows you to reuse your
existing user session and avoid a second login prompt.
:::
...
Token exchange validates the external token by calling the [OpenID Connect
userinfo endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo)
configured for the application account. A root administrator can set the
account's `oidc_userinfo_endpoint` when creating the account or update it with
the `PATCH /accounts/{id}` API:

```shell
curl --request PATCH \
  --url "https://platform.vertexvis.com/accounts/[ACCOUNT_ID]" \
  --header "Authorization: Bearer [ROOT_ACCESS_TOKEN]" \
...
The value must be an absolute HTTPS URL without embedded credentials or a
fragment. HTTP endpoints are accepted only in local development and platdev
environments. The account setting takes precedence over the deployment's
`OidcUserinfoEndpoint` CloudFormation parameter. If the account does not have an
endpoint, token exchange uses that deployment fallback. Omitting the field from
an account update preserves the existing account setting.
