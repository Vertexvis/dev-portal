...
There isn't much information in the event for security purposes. To retrieve additional information on the resource, make a `GET` call to the Vertex API using the `resource.links.related` URL.
