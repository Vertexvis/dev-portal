...
In the case of CRUD events,
more information about the resource attributes or associated resources will be present in the `included` resource array. For Update
events specifically, the `data.attributes.changed` field will contain a list of properties on the resource that have changed.
