...
exchange](../guides/authentication.md).
:::
...
- Threads and replies **require a user access token** obtained through an
  RFC8693-compliant [token exchange process](../guides/authentication.md).
...
- Use an application token to manage collaboration contexts.
- Use a user token (via token exchange) to post or read threads and replies.
- For more details, including how to set up identity provider integration and
  perform a token exchange, see the Authentication Guide.
