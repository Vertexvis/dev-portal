...
You gain secure access to Vertex APIs by exchanging OAuth 2.0 credentials for an access token. Subsequent Postman calls will then include the access token in the `Authorization` header automatically.
